...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    try {
      ...
      console.warn('Gemini 2.5-flash failed, attempting fallback model:', primaryErr?.message);
    ...
    return res.json({
      reply: replyText || generateFallbackResponse(userPrompt, context),
      source: 'gemini',
      model: usedModel,
    });
  } catch (error: any) {
    ...
    const fallbackReply = generateFallbackResponse(lastMessage?.content || '', context);
